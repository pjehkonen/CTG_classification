...
def train_test_split(pdg, my_env, logger, classifier, start_time, X, y, split=0.2):
    ...
    logger.info(
        "Writing X training set indices to file {}".format(classifier + '/log/' + start_time + '/X_train.indices'))
    logger.info("Writing X test set indices to file {}".format(classifier + '/log/' + start_time + '/X_test.indices'))
    logger.info(
        "Writing y training set indices to file {}".format(classifier + '/log/' + start_time + '/y_train.indices'))
